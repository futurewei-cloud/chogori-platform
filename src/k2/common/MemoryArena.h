...
{
//
//  Plain and simple memory arena. TODO: need to do reasonable allocation or use Facebook Polly
//
class MemoryArena
{
    struct Chunk
    ...
    static constexpr size_t chunkSizes[] { 1024, 4096, 16384, 65536, 262144, 1048576, 4194304, 16777216};  // TODO: consider "size" variable which prefixes each buffer
...
public:
    void* alloc(size_t size)
    {
        ...
        if(currentChunk.data == nullptr || currentChunk.used + size > currentChunk.size)
            allocNewChunk(size);
