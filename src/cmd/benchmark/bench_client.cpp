...
public:
    ...
    public:
        ...
        future<size_t> rxrx()
        {
            return do_write(tx_msg_nr)
                .then([this] { return _write_buf.close(); })
                .then(
                    [this] { return make_ready_future<size_t>(_bytes_write); });
        }
    };
    ...
    future<> rxrx_test(connection *conn)
    {
        ...
        return conn->rxrx().then([started](size_t bytes) {
            auto finished = myRDTSC();
            clients.invoke_on(0, &client::rxtx_report, started, finished,
                              bytes);
        });
    }
    ...
    void ping_report(uint64_t started, uint64_t finished)
    {
        ...
        if (++_num_reported == _concurrent_connections) {
            ...
            fprint(std::cout, "started %lu finished %lu\n", _earliest_started,
                   _latest_finished);
            auto secs =
                static_cast<double>(usecs) / static_cast<double>(1000 * 1000);
            ...
            clients.stop().then([] { engine().exit(0); });
        }
    }
        ...
        for (unsigned i = 0; i < ncon; i++) {
            ...
            engine()
                ...
                .then([this, test](connected_socket fd) {
                    ...
                    (this->*tests.at(test))(conn).then_wrapped([conn](
                                                                   auto &&f) {
                        delete conn;
                        try {
                            f.get();
...
int main(int ac, char **av)
{
    ...
    app.add_options()("server", bpo::value<std::string>()->required(),
        ...
        "cyclesPerSec", bpo::value<double>()->default_value(3493470000),
        "Cycles per second, get from dmesg and TSC")(
        "msg_size", bpo::value<int>()->default_value(64),
    ...
    return app.run_deprecated(ac, av, [&app] {
        ...
        clients.start().then([server, test, ncon]() {
            clients.invoke_on_all(&client::start, ipv4_addr{server}, test,
                                  ncon);
        });
    });
}
