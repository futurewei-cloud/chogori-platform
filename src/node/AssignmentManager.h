...
protected:
    ...
    std::array<std::pair<PartitionId, std::unique_ptr<Partition>>,
               Constants::MaxCountOfPartitionsPerNode>
        partitions;         //  List of currently assigned partitions
    ...
    Status processPartitionAssignmentMessage(PartitionRequest &request)
    {
        ...
        if (!request.message->getPayload().getReader().read(
                assignMessage)) //  TODO: allocate under partition memory arena
            return LOG_ERROR(Status::MessageParsingError);

        Collection *collection = nullptr;
        RIF(pool.internalizeCollection(
            std::move(assignMessage.collectionMetadata), collection));

        partitions[partitionCount++] =
            std::make_pair(request.message->getPartition().id,
                           std::make_unique<Partition>(
                               std::move(assignMessage.partitionMetadata),
                               *collection, assignMessage.partitionVersion));
        Partition &partition = *partitions[partitionCount - 1].second;

        partition.createAndActivateTask<AssignmentTask>(
            std::move(request.client));

        return Status::Ok;
    }

    Status getAndCheckPartition(std::unique_ptr<PartitionMessage> &message,
                                Partition *&partition)
    {
    ...
    Status processPartitionOffloadMessage(PartitionRequest &request)
    {
        ...
        partition->createAndActivateTask<OffloadTask>(
            std::move(request.client));
    ...
    Status processClientRequestMessage(PartitionRequest &request)
    {
        ...
        partition->createAndActivateTask<ClientTask>(
            std::move(request.client),
            std::move(request.message->getPayload()));
...
public:
    ...
    void processTasks()
    {
        ...
        std::chrono::nanoseconds maxIterationTime =
            NodeConfig::getTaskProcessingIterationMaxExecutionTime();
        TimeTracker iterationTracker(maxIterationTime);

        std::chrono::nanoseconds remainingTime;
        while ((remainingTime = iterationTracker.remaining()) >
               std::chrono::nanoseconds::zero()) {
            std::chrono::nanoseconds maxPartitionTime =
                remainingTime / partitionCount;
            for (int i = 0; i < partitionCount; i++) {
                if (!partitions[i].second->processActiveTasks(
                        maxPartitionTime)) {
                    //  Partition was dropped
                    std::move(partitions.begin() + i + 1,
