...
//  IOOperation type
//
enum class IOOperationType { Persist, Message };
...
public:
    IOOperationType getType() { return type; }
...
struct ModuleResponse {
    enum ResponseType : uint8_t {
        ...
        RescheduleAfterIOCompletion, //  Continue when all IO operations are
                                     //  finished
    };

    ResponseType type;
    union {
        uint32_t resultCode; //  If type == Error: contains Module specific
                             //  result code
        uint32_t
            potponeDelayUs; //  If type == Postpone: contains time in
                            //  microseconds for which task needs to be delayed
    };

    ModuleResponse(ResponseType type, uint32_t value)
        : type(type), resultCode(value)
    {
    }
...
public:
    ...
    //      schedule task again
    //
    virtual ModuleResponse onPrepare(ClientTask &task,
                                     IOOperations &ioOperations) = 0;
    ...
    //
    virtual ModuleResponse onCoordinate(ClientTask &task,
