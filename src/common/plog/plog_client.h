...
typedef struct plog_durability_vec {
    uint16_t total_cnt;
    plog_durability_v2_t
        plog_durability[MAX_DURABILITY_VEC_SIZE]; /* Data Redundancy Mode */
} plog_durability_vec_t;

/**
 * PLog descriptor V2:
 *    the user must use the descriptor to tell Persistence to create the plog
 * with specified performance/durabilit_v2/capacity.
 */
typedef struct plog_descriptor_v2 {
    ...
    affinity_policy_t
        affinity_policy; /* affinity attribute, just for SGL, 16Byte */
...
typedef struct plog_io_guide_info_v2 {
    ...
    uint32_t chunk_size; /* single block size , full stripe =  origin_num *
                            block_size; */

    // uint32_t min_append_block_size; //not used any more.
    // uint32_t preferred_append_block_size;/* full stripe =  origin_num *
    // chunk_size; */ uint32_t max_append_block_size;
...
* Input Parameters:
            ...
            @plog_expired_time: timeout value use default value when passing 0,
default expire time is 30Days,��λΪmin
            @create_option:
...
* Comments:
*******************************************************************************/
int create_plog(const plog_descriptor_t *descriptor, const uint32_t cnt,
                const uint32_t plog_expired_time,
                const create_option_t *create_option,
                const uint32_t timeout_in_ms, void *trace_info,
                plog_vec_t *plogs);
...
* Input Parameters:
            ...
            @trace_info: used for debugging
            @plog_expired_time: timeout value use default value when passing 0,
default expire time is 30Days,??��?min
            @create_option:
            @timeout_in_ms: timeout
            @trace_info: used for debug
...
* Comments:
*******************************************************************************/
int create_plog_v2(const plog_descriptor_v2_t *descriptor, const uint32_t cnt,
                   const uint32_t plog_expired_time,
                   const create_option_t *create_option,
                   const uint32_t timeout_in_ms, void *trace_info,
                   plog_vec_v2_t *plogs);
...
* Comments:
*******************************************************************************/
int create_plog_async(const plog_descriptor_t *descriptor, const uint32_t cnt,
                      const uint32_t plog_expired_time,
                      const create_option_t *create_option,
                      const uint32_t timeout_in_ms, void *trace_info,
                      plog_vec_t *plogs, const plog_call_back_t *cb);
...
* Comments:
*******************************************************************************/
int get_plog_info(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                  void *trace_info, plog_info_t *plog_info);

int get_plog_info_v2(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                     void *trace_info, plog_info_v2_t *plog_info);
...
* Comments:
*******************************************************************************/
int get_plog_info_async(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                        void *trace_info, plog_info_t *info,
                        const plog_call_back_t *cb);
...
* Comments:
*******************************************************************************/
int seal_plog(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
              void *trace_info);
...
* Comments:
*******************************************************************************/
int seal_plog_async(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                    void *trace_info, const plog_call_back_t *cb);

/*******************************************************************************
* Function Description: To append plog synchronously.
            1. Buffer size is limited to 2M when appending under MULTICOPY
redundancy mode.
...
* Input Parameters:
            @plog_id: PlogId
            @buffer_list: list of buffers, return the offset where stores first
buffer
...
* Comments:
*******************************************************************************/
int append_plog(const plog_id_t *plog_id, const plog_buffer_list_t *buffer_list,
                const append_option_t *append_option,
                const uint32_t timeout_in_ms, void *trace_info,
                uint64_t *offset);
...
* Comments:
*******************************************************************************/
int append_plog_async(const plog_id_t *plog_id,
                      const plog_buffer_list_t *buffer_list,
                      const append_option_t *append_option,
                      uint32_t timeout_in_ms, void *trace_info,
                      uint64_t *offset, const plog_call_back_t *cb);

/*******************************************************************************
* Function Description: To read data from one plog.
            1. Support reading multiple positions within one plog which return
the data into buffer lists.
            2. Buffer size is limited to 2M when reading data under MULTICOPY
redundancy mode.
            3. For EC redundancy mode, limit is 2M* #of stripe.
            4. offset must be the start of crc, otherwise error will be
returned.
...
* Input Parameters:
            @plog_id:  PlogId
            @data_v: Offset and length offset must be the start of crc, or
return err
...
* Comments:
*******************************************************************************/
int read_plog(const plog_id_t *plog_id, const plog_data_to_read_vec_t *data_v,
              const read_option_t *read_option, const uint32_t timeout_in_ms,
...
* Comments:
*******************************************************************************/
int read_plog_async(const plog_id_t *plog_id,
                    const plog_data_to_read_vec_t *data_v,
                    const read_option_t *read_option,
                    const uint32_t timeout_in_ms, void *trace_info,
                    plog_buffer_list_t *buffer_list,
                    const plog_call_back_t *cb);

/*******************************************************************************
* Function Description: To delete the data and metada on Plog disks
synchronously.
            1. if not return P_OK for deletion, Data in persistence layer could
be fully deleted, partially deleted or not deleted at all Delete sealed plog
only
...
* Comments:
*******************************************************************************/
int delete_plog(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                void *trace_info);
...
* Comments:
*******************************************************************************/
int delete_plog_async(const plog_id_t *plog_id, const uint32_t timeout_in_ms,
                      void *trace_info, const plog_call_back_t *cb);
...
* Comments:
*******************************************************************************/
int query_durability(const plog_perf_type_e type, void *trace_info,
                     plog_durability_vec_t *durability_vec);

/* gc ���ʹ�øýӿ�,  ��ѯָ�������������ʣ������*/
int query_capacity_v2(const plog_perf_type_e type,
                      const plog_durability_v2_t durability, void *trace_info,
