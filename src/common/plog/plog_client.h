...
* Input Parameters: 
            ...
            @plog_expired_time: timeout value use default value when passing 0, default expire time is 30Days,单位为min
            @create_option:
...
* Input Parameters: 
            ...
            @cnt: number of plog to create, maxmum: MAX_PLOG_NUM_PER_CREATION
            @trace_info: used for debugging
            @plog_expired_time: timeout value use default value when passing 0, default expire time is 30Days,??λ?min
            @create_option:
            @timeout_in_ms: timeout
            @trace_info: used for debug
