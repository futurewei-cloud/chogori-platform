 ...
 *   just for SGL
 *   affinity policy:
 *   the user must use the policy to tell Persistence to create the plog with
 * affinity.
 */
...
typedef enum cache_ctrl_flag {
    CACHE_PRIORITY_WRITE_READ_DEFAULT =
        0x0000, /* read write read cache default. */
    CACHE_PRIORITY_WRITE_THROUGH_IDX =
        0x0002, /* write cache write through idx */
    CACHE_PRIORITY_WRITE_RESIDENT_IDX =
        0x0004,                            /* write cache write resident idx */
...
typedef enum plog_ret_code {
    ...
    P_PLOG_SEALED = -99998,  /* The plog is sealed thus any append request would
                                be rejected. */
    ...
    P_EXCEED_PLOGID_LIMIT =
        -99993, /* Exceeds the limit of PLogIDs on single node. */
    P_PERF_AND_DUR_NOT_SUPPORTED =
        -99992, /* The PLog pool with specified perf type or durability level is
                   not supported. */
    P_PLOG_CORRUPTED = -99991, /* The PLog has corrupted permanently and only
                                  parts of the data can be obtained. */
    P_PLOG_UNAVAILABLE =
        -99990, /* The PLog is unavailalbe. Recovery is not guranteed. */
    P_PLOG_IS_SEALLING = -99989, /* The PLog is sealing and cannot be accessed.
                                    Please retry later. */
    P_PLOG_UNSEALED = -99988,
    P_PLOG_CLIENT_NOT_READY = -99987, /* PLOG Client not ready yet */
    P_PLOG_CLIENT_UNAVAILABLE =
        -99986, /* canot find  PLOG Client service.  PLOG Client maybe down*/
    P_NAMESPACE_ID_INVALID =
        -99985, /* check namespaceid fail:namespaceid in req to plog server is
                   different from namespaceid in vdb*/
 ...
 * 2. user only specifies plog perf type and plogmanager will allocate the plogs
...
typedef enum plog_perf_type {
    ...
    PL_PERF_TYPE_HDD_WITH_POWER_SAVING =
        2, /* Media type such as SMR, that used for cold storage and could do
              power saving */
 ...
 * 2. erasure-coding supports different types as well. In order to choose EC
 * type, user specifies the AZ level(single/two_az/tri_az) and plogmanager will
 * dynamically choose which N+M to use based on comprehensive analysis of
...
/**
 * PLog Capacity:
 *   Specified numbers are just suggestions, while the real size of a plog will
 * depend on the media capacity. Taking SSD as the storage media, if user
 * requests 64MB plog size while the block size of SSD is 96MB, Persistence
 * would allocate 96MB plog for this request. User may gain performance benefit
 * when the SSD can erase the entire block when delete the plog.
 */
...
/**
 * PLog descriptor:
 *   the user must use the descriptor to tell Persistence to create the plog
 * with specified performance/durability/capacity.
 */
...
/**
 * Plog io guide info:
 *   the folowing table defines the block size of append operations in
 Persistence.
 *   Multi-copy supports byte level append, while erasure-coding only support
 full stripe(512K*N) level append now.
 * !!!Note
 *   For erasue-coding, user can only prepare the buffer with
 min_append_block_size.
 *   Persistence will process the buffer(split to N chunks and do erasure coding
 internal).
    ------------------------------------------------------------------------------------------------------
    |            filed                |    multi-copy(ssd) |erasure
 coding(HDD,N+M) |
    ------------------------------------------------------------------------------------------------------
    |    min_append_block_size        |    1B(fixed size) |        512K*N |
    ------------------------------------------------------------------------------------------------------
    |    preferred_append_block_size  |    4K(variable size,decided by device
 type)  |        512K*N             |
    ------------------------------------------------------------------------------------------------------
 ...
 * Stable state of one plog, and the state changes as:
 ...
 PLOG_UNSEAL -------> PLOG_NORMAL_SEALED
    ...
    the state change is non-reversible and the executable io operation must
 follow this table:

    ---------------------------------------------------------------------------
    |    state\operation            | append |    read | delete | seal |
 getploginfo |
    ---------------------------------------------------------------------------
    |    PLOG_UNSEAL                |    Y     |     Y     |     N      |     Y
 |      Y            |
    ---------------------------------------------------------------------------
    |    PLOG_NORMAL_SEALED        |    N    |     Y     |     Y      |     N |
 Y            |
    ---------------------------------------------------------------------------
...
typedef enum plog_state {
    ...
    PLOG_ABNORMAL_SEALED = 1, /* abnormal seal. for example: seal when layer
                                 Persistence Layer failed */
    PLOG_NORMAL_SEALED = 2,   /* normal sealed */
    PLOG_SEALING = 3, /* plog is in progress of sealing, can not append and
                         query will not give consistent result */
} plog_state_e;
...
/**
 * PLog ID:
 * A plog is a append-only data store unit, and each plog has an id which is
...
typedef struct plog_info {
    ...
    uint32_t
        plog_expired_time; /* time of the plog expired that would be sealed*/
} plog_info_t;
...
typedef enum plog_create_flag {
    ...
    PLOG_CREATE_EXACT_FLAG =
        0x0004, /* If set as 1, means persistence must create plog with specifed
                   durability, not use */
...
typedef enum plog_append_flag {
    ...
    PLOG_FLAG_APPEND_PARTIAL_EC_STRIPE =
        0x0001, /* only partial ec stripe data that append into persistence
                   layer, and will be padded as 0*/
    ...
    // TEST ONLY
    TEST_PLOG_FLAG_APPEND_MOCK_DATA =
        0x8000, /* test only. For fast store data, the data would not be
                   transfered through the NewWork, and will store as 0 on the
                   storage device*/
} plog_append_flag_e;

typedef enum plog_read_flag {
    PLOG_FLAG_READ_DEFAULT = 0x0000,
    PLOG_FLAG_READ_REPLICA_WHEN_ERROR =
        0x0001, /* read data from other replica when error occurs, this could
                   miss some data that is processing on Primary node. */

    PLOG_FLAG_READ_WITHOUT_CHECK_CRC = 0x0002, /* do not verify crc on read */
    PLOG_FLAG_READ_WITHOUT_CACHE =
        0x0080, /* do not put data in cache on read, just for SGl*/
} plog_read_flag_e;
...
typedef struct plog_userid_stru {
    uint32_t userid_type; /* userid type��for exampe,used to indicate the type
                             of database */
    uint32_t userid_len; /* valid data length in userid_buf,for database,use 32
                            bytes UUID*/
...
* Comments:
*******************************************************************************/
int query_capacity(const plog_perf_type_e type,
                   const plog_durability_e durability, void *trace_info,
