...
template <typename T>
struct IsPayloadSerializableTypeTrait<
    T, typename enable_if_type<
           typename T::__K2PayloadSerializableTraitTag__>::type>
    : std::true_type {
};
...
template <typename T>
struct IsPayloadCopyableTypeTrait<
    T, typename enable_if_type<typename T::__K2PayloadCopyableTraitTag__>::type>
    : std::true_type {
};

template <typename T> //  Type that can be just copy to/from payload, though may
                      //  not have reference
constexpr bool isNumbericType()
{
    return std::is_arithmetic<T>::value || std::is_enum<T>::value;
}

template <typename T> //  Type that can be just copy to/from payload (e.g.
                      //  integers, enums, etc.)
constexpr bool isPayloadCopyableType()
{
    return IsPayloadCopyableTypeTrait<T>::value;
}

template <typename T> //  Type that need custom serialization to convert to/from
                      //  payload
constexpr bool isPayloadSerializableType()
{
    return IsPayloadSerializableTypeTrait<T>::value;
}

//
//  Payload is abstraction representing message content. Underneath can
//  reference several buffers. Due to that using indexer can be not efficient.
//  For efficient sequential reading uses PayloadReader (getReader).
//
class Payload
{
...
protected:
    ...
    Position navigate(size_t offset) const
    {
        ...
        for (uint32_t i = 0; i < buffers.size();
             i++) //  TODO: use binary search on separate array
        {
    ...
    bool allocateBuffer()
    {
        buffers.push_back(std::move(
            Binary(8096))); //  TODO: in DPDK case allocate from NIC buffer pool
        return true;
    }

public:
    Payload() : size(0) {}
    Payload(std::vector<Binary> buffers, size_t size)
        : buffers(std::move(buffers)), size(size)
    {
    }
    ...
    uint8_t operator[](size_t index) const { return getByte(index); }
...
//
//  PayloadReader helps to navigate through the payload and read the message.
//  Create by the Payload
//
class PayloadReader
{
...
protected:
    ...
    PayloadReader(const Payload &payload, Payload::Position position)
...
public:
    bool isEnd() const { return position.buffer == payload.buffers.size(); }
    ...
    bool read(Binary &binary, size_t size)
    {
        ...
        //  Need to copy
        binary = Binary(size); //  TODO: probably need to check size condition
    ...
    std::enable_if_t<isPayloadCopyableType<T>() || isNumbericType<T>(), bool>
    read(T &value)
    {
...
//  TODO: we may want to give users buffer awareness, so they can fit their
//  object within single buffer. In this case, even buffers in the middle of
//  payload may have different lengths.
//
class PayloadWriter
{
    friend class Payload;
...
protected:
    ...
    bool allocateBuffer() { return payload.allocateBuffer(); }
    ...
    void moveToNextBufferIfNeeded()
    {
        if (position.buffer == payload.buffers.size() - 1 &&
            payload.buffers[position.buffer].size() == position.offset) {
            position.buffer++;
            position.offset = 0;
        }
    }
...
public:
    //
    //  Define writer position just in case we want to have some special data
    //  for writer
    //
    class Position
    {
    ...
    protected:
        ...
        Position(Payload::Position position, size_t offset)
            : position(position), offset(offset)
        {
        }
    };

    PayloadWriter(Payload &payload, size_t offset)
        : payload(payload), offset(offset)
    {
        position = payload.navigate(offset);
    }

    PayloadWriter(Payload &payload,
                  const PayloadWriter::Position &writerPosition)
        : payload(payload), position(writerPosition.position),
          offset(writerPosition.offset)
    {
    }
    ...
    bool write(const void *data, size_t size)
    {
        ...
        while (size > 0) {
            Binary &buffer =
                const_cast<Binary &>(payload.buffers[position.buffer]);

            size_t currentBufferRemaining = buffer.size() - position.offset;
            size_t needToCopySize = std::min(size, currentBufferRemaining);

            std::memcpy(buffer.get_write() + position.offset, data,
                        needToCopySize);
    ...
    {
        if (position.buffer < payload.buffers.size() - 1)
            payload.buffers.erase(payload.buffers.begin() + position.buffer + 1,
                                  payload.buffers.end());

        payload.size = offset;
    }

    Position getCurrent() const { return Position{position, offset}; }
    ...
    template <typename T> //  Read for primitive types - need copy here to get
                          //  address
