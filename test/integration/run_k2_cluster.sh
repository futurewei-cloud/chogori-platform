...
CPO_ADDITIONAL_PORT=${CPO_ADDITIONAL_PORT:=9001}
PROMETHEUS_PORT_START=${PROMETHEUS_PORT_START:=63000}

# start CPO
cpo_main ${COMMON_ARGS} -c1 --tcp_endpoints ${CPO} ${CPO_ADDITIONAL_PORT} --data_dir ${CPODIR} --prometheus_port $PROMETHEUS_PORT_START --assignment_timeout=1s --txn_heartbeat_deadline=1s --nodepool_endpoints ${EPS[@]} --tso_endpoints ${TSO} --tso_error_bound=100us --persistence_endpoints ${PERSISTENCE}&
cpo_child_pid=$!
