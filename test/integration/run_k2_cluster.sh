...
# start http proxy
http_proxy ${COMMON_ARGS} -c1 --tcp_endpoints ${HTTP} --memory=${HTTPMEM} --cpo ${CPO} --httpproxy_txn_timeout=100h --httpproxy_expiry_timer_interval=50ms --log_level=Info  --prometheus_port $(($PROMETHEUS_PORT_START+4)) &
http_child_pid=$!
