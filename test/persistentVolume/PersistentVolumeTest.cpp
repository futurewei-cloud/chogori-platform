...
IOResult<std::shared_ptr<PersistentVolume>> createVolume(std::string test_name)
{
    return PersistentVolume::create(std::make_shared<PlogMock>(plogBaseDir+test_name));
}

IOResult<std::shared_ptr<PersistentVolume>> initVolumeTest(std::string test_name)
{
